...
<details>
  ...
  - **회원/사용자 관리**  
      일반 회원가입 (이메일, 비밀번호, 닉네임)<br>
      SNS 회원가입 (네이버 OAuth 연동)<br>
      마이페이지: 개인정보 확인 및 수정 (비밀번호 변경, 연락처 등)<br>
      이력서 등록 (입력 기반)

  - **공고 추천 기능**  
      AI 추천: 이력서 기반 자동 추천 / 프롬프트 기반 맞춤 추천<br> 
      추천 UI: 회원은 이력서 기반 추천, 비회원은 예시 공고 노출<br>
      찜한 공고 기반 재추천 (선호 로그 반영)<br>
      예시 프롬프트 제공 (추천/검색/로드맵)

  - **채용공고 검색/조회**  
      전체 공고 검색 (직무, 키워드, 기업명, 지역 등 복수 조건 필터링)<br>
      상세 페이지 연동 및 찜 기능<br>
      페이지네이션, 정렬 기능 (적합도순, 마감일순 등)
    
  - **커리어 로드맵 분석 및 시각화**  
      직무 트렌드 시각화 (기술스택, 툴 등 워드클라우드/막대차트)<br>
      프롬프트 기반 직무 매칭 시각화<br>
      내 이력서와 직무 간 갭 분석 (기술/자격요건/툴 기준)<br>
      갭 기반 개인화 커리큘럼 제공 (투두리스트와 연동)

  - **To-do List 기능**  
      캘린더 기반 일정 관리 기능<br>
      GPT 기반 자동 생성 일정/체크리스트 지원<br>
      프롬프트 기반 일정 추천 기능<br>
      커리큘럼과 연동된 액션플랜 제공
    
  - **기타 UI 기능**
      사이드바 메뉴 구성 및 페이지 이동<br>
...
<details>
  ...
  - **성능**  
      1,000명 이상의 동시 접속자 처리 가능<br>  
      FastAPI + PostgreSQL + Redis 기반의 비동기 처리 및 캐싱 적용
    
  - **보안**  
      사용자 및 공고 데이터 암호화 저장<br>  
      JWT 인증 및 HTTPS 기반 보안 통신<br>
      비밀번호는 해시 처리, 로그인 실패 시 보안 조치 적용

  - **확장성**  
      향후 외부 채용 플랫폼 공고 연동 고려 (API 기반)<br>
      다양한 직무 카테고리 및 커리큘럼 데이터 추가 용이
    
  - **사용성**  
      반응형 웹 UI로 PC/모바일 환경 모두 대응<br>
      비회원은 일부 기능 제한 및 로그인 유도<br>
